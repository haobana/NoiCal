 ...
 * 返回值： bool
 *****************************************************************************/
bool WordEngine::openAndCopy(bool bVisable)
{
    //新建一个word应用程序,并设置为可见
 ...
 * 返回值：bool
 *****************************************************************************/
bool WordEngine::openAndCopy(const QString& strFilePath, bool bVisable)
{
    m_strFilePath = strFilePath;
    close();
    return openAndCopy(bVisable);
}
 ...
 * 返回值：bool
 *****************************************************************************/
bool WordEngine::open(const QString& strFilePath, bool bVisible)
{
    m_strFilePath = strFilePath;
    close();
    return openDocument(bVisible);
}
 ...
 * 返回值：bool
 *****************************************************************************/
bool WordEngine::openDocument(bool bVisible)
{
    // 新建一个word应用程序，并设置为可见
    m_wordWidget = new QAxObject();
    bool bFlag = m_wordWidget->setControl("word.Application");
    ...
    // 打开指定的文档
    QAxObject *documents = m_wordWidget->querySubObject("Documents");
    if (!documents)
    {
        return false;
    }
    ...
    // 打开指定文件
    m_wordDocuments = documents->querySubObject("Open(const QString&)", m_strFilePath);
    if (!m_wordDocuments)
    {
        return false;
    }
