...
Dialog_rect_damper::Dialog_rect_damper(QWidget *parent, int editRow, const Rect_damper_noi& data) :
...
void* Dialog_rect_damper::getNoi()
{
    return noi == nullptr ? nullptr : noi;
}

void Dialog_rect_damper::calNoise()
{
    if(ui->lineEdit_length->text().isEmpty() || ui->lineEdit_width->text().isEmpty() || ui->lineEdit_air_volume->text().isEmpty() || ui->comboBox_angle->currentIndex() == -1)
    {
        return;
    }
    int L = 0;
    ...
    double area = ui->lineEdit_length->text().toDouble() / 100.0 * ui->lineEdit_width->text().toDouble() / 100.0;
...
//通过厂家信息
void Dialog_rect_damper::on_radioButton_known_clicked()
{
...
//通过经验公式
void Dialog_rect_damper::on_radioButton_formula_clicked()
{
