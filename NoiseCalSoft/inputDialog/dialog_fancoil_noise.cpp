...
#include "ui_dialog_fancoil_noise.h"
#include <cmath>

Dialog_fanCoil_noise::Dialog_fanCoil_noise(QWidget *parent, int editRow,  const FanCoil_noise& data) :
    InputBaseDialog(parent),
    editRow(editRow), // 初始化editRow
    ui(new Ui::Dialog_fanCoil_noise)
{
    ui->setupUi(this);
...
Dialog_fanCoil_noise::~Dialog_fanCoil_noise()
{
    delete ui;
}

//获得noi
void* Dialog_fanCoil_noise::getNoi()
{
    return noi == nullptr ? nullptr : noi;
}

// 槽函数实现
void Dialog_fanCoil_noise::calTotalNoise() {
...
//关闭窗口按钮
void Dialog_fanCoil_noise::on_close_clicked()
{
    this->close();
}

//确认按钮
void Dialog_fanCoil_noise::on_pushButton_confirm_clicked()
{
    ...
    noi->type = ui->lineEdit_type->text();
