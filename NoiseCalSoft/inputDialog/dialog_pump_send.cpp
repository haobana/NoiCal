...
Dialog_pump_send::Dialog_pump_send(QString type, QWidget *parent, int editRow,  const PumpSend_noise& data) :
    ...
    ui(new Ui::Dialog_pump_send)
{
    ...
    DBManager->queryKnownData(pumpSendModel, PUMP_SEND_TUYERE_MODEL, PUMP_SEND_TUYERE_TABLE);
    ...
    /******设置combobox******/
    for(int i = 0; i < pumpSendModel.size(); i++)
    {
        QString s = pumpSendModel[i];
        // 检查是否已经添加过该前缀
...
void* Dialog_pump_send::getNoi()
{
    return noi == nullptr ? nullptr : noi;
}

//计算总值
void Dialog_pump_send::total_noi()
{
...
//计算总值 槽函数实现
void Dialog_pump_send::calReflNoi()
{
    double D = 0;
...
void Dialog_pump_send::on_comboBox_model_currentTextChanged(const QString &arg1)
{
    ...
    DBManager->queryEightNoi(eightNoi, PUMP_SEND_TUYERE_TABLE, PUMP_SEND_TUYERE_MODEL, modelName);
    for(int i = 0; i < 8; i++)
    {
...
void Dialog_pump_send::on_radioButton_known_clicked()
{
...
void Dialog_pump_send::on_radioButton_formula_clicked()
{
    ui->lineEdit_63_refl->clear();
    ui->lineEdit_125_refl->clear();
    ui->lineEdit_250_refl->clear();
...
void Dialog_pump_send::on_pushButton_clicked()
{
    // 使用资源路径
